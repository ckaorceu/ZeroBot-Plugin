...
func init() {
	engine := control.Register("example2", &ctrl.Options[*zero.Ctx]{
		...
		Brief:            "自定义群管",
		Help: "- hello\n- 完全匹配\n- 菜单1 | 新人系统2 | 完全匹配组3| 完全匹配组4 完全匹配组5 | 完全匹配组6 | 完全匹配组7| 完全匹配组8",
	})
	// hello
	engine.OnFullMatch("hello").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		ctx.SendChain(message.Text("hello world!"))
	})

	// matcher
	engine.OnFullMatch("菜单").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		matched := ctx.State["matched"].(string)
		ctx.SendChain(message.Text("完全匹配的匹配词: ", matched))
	})
	engine.OnFullMatchGroup([]string{"新人系统"}).SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		matched := ctx.State["matched"].(string)
		ctx.SendChain(message.Text("新人系统: ", matched))
	})
	...
	// message
	engine.OnFullMatch("菜单").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		ctx.SendChain(message.Text("[bq144]◇━综合插件━◇[bq144]
...
◇━━-欢迎使用-━━◇"))
	...
	engine.OnFullMatch("新人系统").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		ctx.SendChain(message.At(ctx.Event.UserID), message.Text("[bq144]◇━新人系统━◇[bq144]
...
◇━━-欢迎使用-━━◇"))
	...
	engine.OnFullMatch("回复").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		ctx.SendChain(message.Reply(ctx.Event.MessageID), message.Text("回复你"))
	})
	engine.OnFullMatch("图片").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		ctx.SendChain(message.Image("https://profile-avatar.csdnimg.cn/de3b442a33594feb8507dbb9d745e40a_yenange.jpg!3"))
	})
	...
	engine.OnFullMatch("表情").SetBlock(true).Limit(limit.LimitByGroup).Handle(func(ctx *zero.Ctx) {
